...
export function createPngHandlerFromImageData(
  imageData: ImageData
): PngHandler {
...
export class PngHandler {
  ...
  getMatrixOfPixelsFromNdArray(matrix: NdArray) {
    ...
    const reshapedMatrix = ndarray(new Int16Array(matrix as any), [
      this.rows,
      this.columns,
      4
    ]);
  ...
  convertToProofTest(): string[] {
    ...
    const emptyRowColumnsPadding = new Array(this.desiredColumnsLength).fill(
      "0"
    );
  ...
  checkCircomOutput(
    matrixProofInput: string[],
    matrixProofOutput: string[],
    rows: number,
    columns: number
  ): {
    result: boolean;
    ...
    const reversedMatrixTest = this.reverseMatrix(
      this.getMatrixOfPixelsFromNumberArray(this.pixels.flat().flat())
    );

    if (
      unpaddedProofOutput.length !==
      ...
      return {
        ...
        unpaddedProofOutput: [],
        reversedMatrixTest: ndarray([], [0, 0]),
        errorMsg: `Matrices have different dimensions: ${
          unpaddedProofOutput.length
        } and ${reversedMatrixTest.shape[0] * reversedMatrixTest.shape[1]}`
      };
    }
        ...
        for (let pixel = 0; pixel < 4; pixel++) {
          ...
          if (
            ...
            return {
              ...
              unpaddedProofOutput: [],
              reversedMatrixTest: ndarray([], [0, 0]),
              errorMsg: `Matrices have different elements at index ${i}: ${
                unpaddedProofOutput[i]
              } and ${reversedMatrixTest.get(i, j, pixel)}`
            };
          }
        }
      }
  ...
  reverseRows(matrix: string[]): string[] {
    ...
    const matrixInput = ndarray(matrix, [
