...
let png_handler: PngHandler | null;
let isFlipped = false;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const flipButtonHandler = document.getElementById(
    "flipButton"
  ) as HTMLButtonElement;
  const canvas = document.getElementById("imageCanvas") as HTMLCanvasElement;
  const ctx = canvas.getContext("2d") as CanvasRenderingContext2D;

  const copyProofButton = document.getElementById(
    "copyProof"
  ) as HTMLButtonElement;
  const proofOutputElement = document.getElementById(
    "proofOutput"
  ) as HTMLTextAreaElement;

  const verifyProofButton = document.getElementById(
    "verifyProof"
  ) as HTMLButtonElement;
