import {
  ...
  function testSetup(fileExtension: string, desiredRowsLength: number, desiredColumnsLength: number) {
      ...
      try {
        const { proof, publicSignals } = await groth16.fullProve(
          ...
          baseDirectory + '/circuits/zkFiles/involution_final.zkey',
            ...
            consoleErrorEnable(originalConsolError);
            expect((error as Error).message).toContain('Error in template VerifyImageBounds_5 line: 54');
        }
    });
